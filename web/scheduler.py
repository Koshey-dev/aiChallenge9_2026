...
Инструменты — те же записи, что у трекера дня 17: имя, описание, схема и
обработчик. С дня 20 у планировщика свой сервер (`SERVER`, протокол из
`tracker.Server`), а подсказка с «сейчас» — его `instructions`. Обработчику
приходит и чат, из которого его позвали: задания принадлежат чату, и вызов
из другого чата их не видит. Сам цикл про модель не знает — текст сводки он просит у функции,
...
SERVER = tracker.Server("aichallenge-scheduler", hint, TOOLS)
