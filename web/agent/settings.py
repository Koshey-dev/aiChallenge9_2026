...
BLOCKS = [
        ...
        "fields": [
            {"key": "strategy", "label": "Стратегия", "type": "choice",
             ...
             "options": [
                 ...
                 {"value": "layers",
                  "label": "Слои — короткая, рабочая и долговременная память"},
             ],
             "hint": "окно — основа для остальных: факты добавляют к нему карточку, "
                     "слои — рабочую память и профиль, а ветки меняют не состав "
                     "запроса, а линию диалога, из которой берётся история"},
    ...
    {
        "title": "Модель памяти",
        "note": "Три слоя, разделённые сроком жизни: короткая — реплики в окне "
                "памяти, рабочая — данные текущей задачи, долговременная — "
                "профиль пользователя. Работает при стратегии «Слои».",
        "fields": [
            {"key": "send_work", "label": "Слать рабочую память в запрос",
             "type": "bool", "default": True,
             "hint": "выключено — слой продолжает заполняться, но в запрос "
                     "не уходит: видно, на что он влияет в ответе"},
            {"key": "send_profile", "label": "Слать долговременную память в запрос",
             "type": "bool", "default": True},
            {"key": "work_max", "label": "Предел рабочей памяти, символов",
             "type": "number", "default": 1200},
            {"key": "profile_max", "label": "Предел долговременной памяти, символов",
             "type": "number", "default": 800,
             "hint": "профиль дополняется, а не переписывается заново: "
                     "лишнее убирается кнопкой в слое"},
        ],
    },
    {
