...
# текстом (`[стек] Хранилище — только PostgreSQL`) и туда же попадает
# к аудитору; на то, как проверяется и переписывается ответ, он не влияет.
# Разной строгости по видам здесь нет намеренно: инвариант либо действует,
# либо выключен, и второй шкалы к нему не нужно.
#
# `about` — только для выпадающего списка в панели: модель его не видит.
# Поэтому там пример, а не определение: «границы между частями» не объясняет
# ничего тому, кто впервые заводит инвариант.
KINDS = [
    {"id": "architecture", "title": "архитектура",
     "about": "границы между частями: «сервисы общаются только через очередь»"},
    {"id": "decision", "title": "решение",
     "about": "принятое и не пересматриваемое: «остаёмся на JS, на TypeScript "
              "не переходим»"},
    {"id": "stack", "title": "стек",
     "about": "чем можно и чем нельзя пользоваться: «хранилище только PostgreSQL»"},
    {"id": "business", "title": "бизнес-правило",
     "about": "правило предметной области: «скидка не больше 10%»"},
]
